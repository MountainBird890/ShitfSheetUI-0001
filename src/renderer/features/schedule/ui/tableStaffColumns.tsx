...
export default function StaffColumn() {
  ...
  // ご利用者ごとの行データを生成
  // { user: "田中", "2026-04-19": { name: "兵庫太郎", start: "10:00", end: "14:00", type: "身体" }, ... }
  const tableData = Array.from(userSet).map((user) => {
    ...
    [...Array(daysInMonth)].forEach((_, index) => {
      ...
      const matched = allStaff.find(
        (staff) => staff.details?.[dateStr]?.user === user
      );
      if (matched) {
