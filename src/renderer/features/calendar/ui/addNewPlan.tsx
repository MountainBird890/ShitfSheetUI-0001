...
import { useAdd } from "../state/useCalendar";
import type { EditFormValues, ScheduleEntry, StaffRecord } from "../type/type";
...
const ScheduleEditModal: React.FC = () => {
  const { open, staff, dateKey, handleSave, closeEditor } = useAdd();
  ...
  useEffect(() => {
    ...
    // 初回読み込み後にbase.jsonに保存されてる値をセットする
    // 今日はココを消去し、新規追加ロジックを作るとこから始める。AntdのFormを使って実装。参考はcalendarのStaffForm.tsx
    form.setFieldsValue({
